...
def comparator(T, data, VF):
    """Simulate the comparator circuit
    """
    R1, R2, VZ, VCC = data['Rref1'], data['Rref2'], data['VZ'], data['VCC']
    threshold = VZ*R2/(R1+R2)
    VOUT = np.zeros(len(T))
    VOUT[VF < threshold] = VCC
    return T, VOUT, threshold
